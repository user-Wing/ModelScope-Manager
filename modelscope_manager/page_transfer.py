...
class TransferPageMixin:
    ...
    def _build_transfer_page(self) -> QWidget:
        ...
        self.download_table.setColumnWidth(0, 360)
        self.download_table.setColumnWidth(1, 420)
        self.download_table.setColumnWidth(2, 180)
        self.download_table.setTextElideMode(Qt.TextElideMode.ElideNone)
        ...
        history_actions.addWidget(QLabel("保存在本机数据库中的全部传输完成记录。", objectName="subtitle"))
        history_actions.addStretch()
        refresh_history = QPushButton("刷新")
        refresh_history.clicked.connect(self._refresh_transfer_history)
        history_actions.addWidget(refresh_history)
        history_layout.addLayout(history_actions)
        self.transfer_history_table = QTableWidget(0, 7)
        self.transfer_history_table.setHorizontalHeaderLabels(["方向", "原始文件/目录", "目标", "结果", "耗时", "完成时间", "详情"])
        history_header = self.transfer_history_table.horizontalHeader()
        history_header.setSectionResizeMode(QHeaderView.ResizeMode.Interactive)
        history_header.setMinimumSectionSize(70)
        history_header.setStretchLastSection(False)
        for column, width in enumerate((80, 300, 300, 100, 100, 170, 300)):
            self.transfer_history_table.setColumnWidth(column, width)
        self.transfer_history_table.setEditTriggers(QTableWidget.EditTrigger.NoEditTriggers)
        history_layout.addWidget(self.transfer_history_table)
        self.queue_tabs.addTab(history_page, "历史记录")
